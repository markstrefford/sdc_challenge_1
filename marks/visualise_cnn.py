...
print("Number of kept filters = {}".format(len(kept_filters)))

# fill the picture with our saved filters
# for i in range(m):
#     for j in range(n):
#         img, loss = kept_filters[(i - 1) * n + j]
#         stitched_filters[(img_width + margin) * i: (img_width + margin) * i + img_width,
#                          (img_height + margin) * j: (img_height + margin) * j + img_height, :] = img
#
# # save the result to disk
# imsave('stitched_filters_%dx%d.png' % (n, m), stitched_filters)

for i in range(n):
    img, loss = kept_filters[i]
    filename = 'filter_{}.jpg'.format(i)
    print("Saving img {} to filename {}".format(i, filename))
